...
from easy_io.backends import BaseStorageBackend, Boto3Backend, HTTPBackend, LocalBackend, MSCBackend
...
class FileClient:
    ...
    _prefix_to_backends: ClassVar[dict[str, type[BaseStorageBackend]]] = {
        "s3": Boto3Backend,
        "http": HTTPBackend,
        "https": HTTPBackend,
    }

    _instances: ClassVar[dict[str, "FileClient"]] = {}
    ...
    def infer_client(
        ...
        if file_client_args is None:
            if uri is None:
                raise ValueError("uri must be provided when file_client_args is None")
    ...
    @classmethod
    def _register_backend(cls, name, backend, force=False, prefixes=None):  # noqa: C901
